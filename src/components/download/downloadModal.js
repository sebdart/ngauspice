...
// Added by S.D. August 2020: //
export const thankyouDOD = {
  nga: {
    agency: "NGA",
    title: "Data Corps, DCD-Intel",
    Fname: "National Geospatial-Intelligence Agency",
    loca: "St Louis, MO",
    href: "https://www.nga.mil"
  },
  hpc: {
    agency: "DOD",
    title: "High Performance Computing Modernization Program",
    Fname: "US Department of Defense",
    loca: "HPC.mil",
    href: "https://www.hpc.mil"
  }
};
...
class DownloadModal extends React.Component {
  ...
  getRelevantThankyouDOD() {
    const x = [thankyouDOD.nga, thankyouDOD.hpc];
    return x;
  }
  formatThankyouDOD(pubs) {
    return (
      <span>
        <ul>
          {pubs.map((pub) => (
            <li key={pub.href}>
              <a href={pub.href} target="_blank" rel="noreferrer noopener">
                {pub.agency}, {pub.title}, <i>{pub.Fname}</i> ({pub.loca})
              </a>
            </li>
          ))}
        </ul>
  ...
  render() {
    ...
    // Added by S.D. August 2020 in formatThankyouDOD: //
    return (
      <div style={infoPanelStyles.modalContainer} onClick={this.dismissModal}>
        <div style={panelStyle} onClick={(e) => stopProp(e)}>
          ...
          <div style={infoPanelStyles.modalSubheading}>
            {t("Please, acknowledge the Department of Defense (DOD) teams")+":"}
          </div>
          {this.formatThankyouDOD(this.getRelevantThankyouDOD())}
